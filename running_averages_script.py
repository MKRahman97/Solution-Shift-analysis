...
def bootstrap(df, numSample, atom):
    ...
    ran_num = randomstate.choice(10**6, 1000, replace=False, shuffle=False)
    for sample, num in zip(range(1,numSample+1), ran_num):
        # Take M sampels from the data frame
...
def _Transpose_df(df):
    ...
    df_T['file_num'] = df_T['index'].str.extract('(\d+)').astype(int)
...
def plot_3D(Dof_df, df):
    ...
    fig = px.scatter_3d(df_melt, x = DOF_X, y= DOF_Y, z = 'value', color='variable')
    fig.show()
