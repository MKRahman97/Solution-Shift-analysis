...
'''
Notes:
Proton labels are note read Need to fix a bug which the averaged proton labels are ignored
'''
...
class Scoring_Function:
    ...
    def __init__(self, master):
        ...
        self.tabControl.add(self.tab3, text='SF each mol', state=tk.DISABLED)
        self.tabControl.add(self.tab4, text='SF Z', state=tk.DISABLED)
    ...
    def open_output(self):
        dir = askdirectory()
        self.path = str(glob.glob(dir))[1:-1]
        print(self.path)
        if self.path != '':
            print(self.path)
    ...
    def solid_solid(self,df_exp, df_calc, Atom):
        ...
        for col in df_exp.columns:
            ...
            if df.isnull().values.any():
                ...
                outfile = '{}_Solid_exp_Vs_Solid_calc_{}.csv'.format(col, Atom)
                # path1 = os.path.join(self.path[1:-1], outfile)
                df_1 = df_1.append(df)
            # print(path1)
            # df.to_csv(path1, index=None)
        if df_1.empty:
            print('Mismatch between calculated labels and experimental labels')
            ...
            # a = df_1.sort_values(by=['RMSE','Exp'], ascending=True).groupby('Exp').head()
            df_1.sort_values('RMSE', inplace=True)  # Sort by RMSE values
        ...
        for mol in range(0,len(df_exp.columns)):
            for i in range(len(df_calc.columns)):
                ...
                print('r_squared = {:.2f} \n m = {:.2f} \n c = {:.2f} \n p = {:.3f}'.format(r_value, slope, intercept,
                                                                                            p_value))
                rsq.append(round(r_value, 2))
                ml.append(round(slope, 2))
                cl.append(round(intercept, 2))
                pl.append(round(p_value, 3))
            ...
            if save_table=='Yes':
                df.to_csv(self.path + 'scoring_function_1000_%s_%s.csv' % (x_title, Atom), index=None)
...
root.mainloop()
#
# emp_1 = Employee('Hanzo', 'Hatori', 1000000)
